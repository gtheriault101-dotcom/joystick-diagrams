...
from joystick_diagrams.plugins.bms.config import (
    settings,  # TODO Move out plugins to separate package
)
from joystick_diagrams.plugins.bms.falcon_bms import (
    FalconBmsPlugin,  # TODO Move out plugins to separate package
)
...
class ParserPlugin(PluginInterface):
    def __init__(self):
        ...
        self.instance: FalconBmsPlugin = None

    def process(self):
        return self.instance.parse()

    def set_path(self, path: Path) -> bool:
        inst = FalconBmsPlugin(path)
    ...
    def path_type(self):
        return self.FolderPath(
            "Select your Falcon BMS configuration folder",
            Path.home()
        )
